function humanTimestamp(format = '{year}-{month}-{day} {hour}:{minute}:{second}.{millisecond} {timezoneOffset}')
{
    let dt = new Date()
    let timestamp = format

    timestamp = timestamp.replace('{year}', String(dt.getFullYear()).padStart(2, '0'))
