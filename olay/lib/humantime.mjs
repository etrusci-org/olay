export function humantime(format, unixtime) {
    const default_format = '{year}-{month}-{day} {hour}:{minute}:{second}.{millisecond} {timezoneOffset}';
    if (!format) {
        format = default_format;
    }
    const dt = (!unixtime) ? new Date() : new Date(unixtime * 1000);
