...
export class Olay_TwitchChat extends Olay {
    ...
    ui = {
        ...
        chat: document.querySelector('.mod .chat'),
        stats: document.querySelector('.mod .stats'),
        statsmessages: document.querySelector('.mod .stats .messages'),
        statschatters: document.querySelector('.mod .stats .chatters'),
        statsduration: document.querySelector('.mod .stats .duration'),
    };
    valid_emotesthemes = ['light', 'dark'];
    stats_messages = 0;
    stats_chatters = 0;
    seen_chatters = {};
    tracking_stats_since = 0;
    TwitchClient;
    constructor() {
        super();
    ...
    on_message(channel, tags, message, self) {
        ...
        const user_color_css = (this.conf.usercolor && tags['color']) ? ` style="color: ${tags['color']};"` : '';
        const timestamp = (!this.conf.timeformat.includes('{beats}')) ? humantime(this.conf.timeformat) : beatsnow(2, this.conf.timeformat);
        ...
        const dump = this.ui.chat.querySelectorAll('.chatline');
