...
export class Olay_Quotes extends Olay {
    ...
    ui = {
        ...
        quote: document.querySelector('.mod .quote'),
        author: document.querySelector('.mod .author'),
    };
    min_updaterate = 10;
    constructor() {
        super();
    ...
    async update_ui() {
        const data = await fetchx('json', this.conf.endpoint, { headers: { 'User-Agent': OLAY_USER_AGENT } });
        ...
        if (this.conf.typingspeed > 0) {
            const queue_quote = this.conf.quoteformat.replace('{quote}', data.quote).split('');
            let iid = setInterval(() => {
                this.ui.quote.innerHTML += queue_quote.shift();
                if (queue_quote.length == 0) {
                    clearInterval(iid);
                    const author_queue = this.conf.authorformat.replace('{author}', data.author).split('');
                    iid = setInterval(() => {
